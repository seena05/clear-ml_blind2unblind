...
class DataLoader_Fmdd_sub(Dataset):
    def __init__(self, data_dir, patch=256):
...
class DataLoader_Medical_Img(Dataset):
    def __init__(self, data_dir, patch=256):
        super(DataLoader_Medical_Img, self).__init__()
        self.data_dir = data_dir
        self.patch = patch
        # self.train_fns = glob.glob(os.path.join(data_dir, '**/raw/**/**.png'), recursive=True)
        self.train_fns = glob.glob(os.path.join(self.data_dir, "*"))         
...
def validation_fmdd(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "raw/**/**.png"), recursive=True)
    fns.sort()
    data_noisy = []
    data_gt = []
    for fn in fns:
        im_noisy = Image.open(fn)
